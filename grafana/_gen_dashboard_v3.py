#!/usr/bin/env python3
"""Generate hummingbot-pair-trade-mobin-v3.json Grafana dashboard."""
import json
...
VER = "13.1.0"
OUT = Path(__file__).with_name("hummingbot-pair-trade-mobin-v3.json")

# Equal-weight mid indexes from signal_snapshot (session start = first mid per symbol).
# Missing mids are forward-filled on a full (bin_ts × symbol) grid.
...
def index_series_sql(label, symbol_filter=None):
    return (
        ...
        f'SELECT time, index_pct AS "{label}" FROM idx ORDER BY time;'
    )
...
def _port_cte():
    return (
        "start_nav AS ("
        "SELECT portfolio_value AS v FROM pair_trade.portfolio_snapshot "
        "WHERE session_id = $session ORDER BY ts ASC LIMIT 1"
        "), "
        "port AS ("
        "SELECT p.ts, 100.0 * p.pnl / NULLIF(sn.v, 0) AS port_pct "
        "FROM pair_trade.portfolio_snapshot p CROSS JOIN start_nav sn "
...
def portfolio_on_signal_bins_sql(label="Portfolio %"):
    """Portfolio P&L % aligned to All-index bin timeline (last portfolio ts ≤ bin_ts)."""
    return (
        f"WITH {_FILLED_MIDS_CTE}, "
        f"idx AS ({_index_avg_sql()}), "
        f"{_port_cte()} "
        f'SELECT i.time, lat.port_pct AS "{label}" '
        f"FROM idx i "
        f"JOIN LATERAL ("
        f"SELECT p.port_pct FROM port p WHERE p.ts <= i.time ORDER BY p.ts DESC LIMIT 1"
        f") lat ON TRUE "
        f"ORDER BY i.time;"
    )
...
def excess_series_sql(label, symbol_filter=None):
    return (
        ...
        f"idx AS ({_index_avg_sql(symbol_filter)}), "
        f"{_port_cte()} "
        f"SELECT i.time, lat.port_pct - i.index_pct AS \"{label}\" "
        f"FROM idx i "
        f"JOIN LATERAL ("
        f"SELECT p.port_pct FROM port p WHERE p.ts <= i.time ORDER BY p.ts DESC LIMIT 1"
        f") lat ON TRUE "
...
# Match palette-classic assignment on the 3-series panel: Portfolio=green, All=yellow, Top5=blue.
...
def main():
    ...
    pie_sql = (
        ...
        "p AS (SELECT ts, portfolio_value, marks FROM pair_trade.portfolio_snapshot "
        "WHERE session_id = $session AND ts = (SELECT ts FROM latest)), "
        "b AS (SELECT asset, SUM(total) AS total FROM pair_trade.balance_snapshot "
        "WHERE session_id = $session AND ts = (SELECT ts FROM latest) GROUP BY asset), "
        "valued AS ("
        "SELECT p.ts AS time, b.asset AS metric, "
        "100.0 * CASE WHEN b.asset = 'IRR' THEN b.total ELSE b.total * COALESCE(("
        "SELECT kv.value::numeric FROM jsonb_each_text(p.marks) AS kv(key, value) "
        "WHERE kv.key = b.asset || '-IRR' OR kv.key LIKE b.asset || '-%' LIMIT 1), 0) END "
        "/ NULLIF(p.portfolio_value, 0) AS value "
        "FROM b CROSS JOIN p"
        ") "
        "SELECT time, value, metric FROM valued WHERE value > 0 ORDER BY value DESC;"
    )
    edges_sql = (
    ...
    elements = {
        ...
        "panel-10c": panel(
            ...
            "Portfolio P&L % (vs session-start NAV) alongside equal-weight mid-price indexes. "
            "All = every symbol in the session; Top5 = عیار، طلا، کهربا، گنج، مثقال. "
            "Index: 100*(mid_ffill / first_mid - 1); missing mids forward-filled. "
            "Portfolio aligned to each signal bin via last portfolio_snapshot.ts ≤ bin_ts.",
            qg([
        ...
        "panel-10d": panel(
            ...
            "Portfolio P&L % (vs session-start NAV) minus each equal-weight mid index %. "
            "Portfolio aligned to each signal bin via last portfolio_snapshot.ts ≤ bin_ts. "
            "Positive = outperforming that benchmark since session start. "
            "Colors match All/Top5 on the panel to the left.",
            qg([
        ...
        "panel-12b": panel(
            ...
            "One slice per asset (time_series + metric); legend shows asset name, value %, and share.",
            qg([pq(pie_sql)]),  # time_series: one series per asset via metric column
        ...
        "panel-30": panel(
            ...
            qg([pq(
                "SELECT NOW() AS time, COUNT(*)::float AS value, status AS metric "
                "FROM pair_trade.rotation WHERE session_id = $session GROUP BY status ORDER BY value DESC;"
            )]),
            pie_viz(),
        ),
        "panel-31": panel(
        ...
        "panel-36": panel(
            ...
            qg([pq(
                "SELECT NOW() AS time, COUNT(*)::float AS value, event_type AS metric "
                "FROM pair_trade.event_log WHERE session_id = $session GROUP BY event_type ORDER BY value DESC;"
            )]),
            pie_viz(),
        ),
        "panel-37": panel(
    ...
    dashboard = {
        ...
        "kind": "Dashboard",
        "metadata": {
            "name": "hummingbot-pair-trade-mobin-v3",
            "namespace": "default",
            "uid": "hb-pair-trade-mobin-v3",
        },
        "spec": {
            "title": "Hummingbot Pair Trade (Mobin Gold) v3",
            "description": (
                "Gold ETF basket: mark-to-mid P&L (% + M IRT), equal-weight mid indexes (All / Top5) "
                "and portfolio excess vs those benchmarks, portfolio weights, signal edges, "
                "rotation execution quality (expected/last/realized/realized_net edge, fees, slippage, latency)."
            ),
            "tags": ["hummingbot", "pair-trade", "mobin", "gold-etf", "v3"],
            "editable": True,
