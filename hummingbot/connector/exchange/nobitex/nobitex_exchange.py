...
class NobitexExchange(ExchangePyBase):
    ...
    def name(self) -> str:
        if self._domain == "com":
            ...
            return f"nobitex_{self._domain}"
    ...
    async def _format_trading_rules(self, exchange_info_dict: Dict[str, Any]) -> List[TradingRule]:
        ...
        retval = []
        for rule in filter(nobitex_utils.is_exchange_information_valid, trading_pair_rules):
            try:
    ...
    async def _user_stream_event_listener(self):
        ...
        async for event_message in self._iter_user_event_queue():
            try:
                ...
                # Refer to https://github.com/nobitex-exchange/nobitex-official-api-docs/blob/master/user-data-stream.md
                # As per the order update section in Nobitex the ID of the order being canceled is under the "C" key
    ...
    def _initialize_trading_pair_symbols_from_exchange_info(self, exchange_info: Dict[str, Any]):
        mapping = bidict()
        for symbol_data in filter(nobitex_utils.is_exchange_information_valid, exchange_info["symbols"]):
