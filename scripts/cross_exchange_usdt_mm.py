...
class CrossExchangeUsdtMmConfig(StrategyV2ConfigBase):
    ...
    maker_trading_pair: str = Field(
        "BTC-USDT",
        json_schema_extra={"prompt": "Maker pair (USDT quote)", "prompt_on_new": True},
    )
    hedge_trading_pair: str = Field(
        "BTC-USDT",
        json_schema_extra={"prompt": "Hedge pair on hedge exchange", "prompt_on_new": True},
    )
...
class CrossExchangeUsdtMm(StrategyV2Base):
    """
    Cross-exchange market making on a USDT maker leg with a USDT hedge (same quote).

    Fair value (USDT): hedge BTC-USDT mid.
    ...
    def __init__(self, connectors: Dict[str, ConnectorBase], config: CrossExchangeUsdtMmConfig):
        super().__init__(connectors, config)
        self.config: CrossExchangeUsdtMmConfig = config
        self._last_fair_usdt: Optional[Decimal] = None
    ...
    def on_tick(self):
        ...
        fair_usdt = self._compute_fair()
        if fair_usdt is None:
            self.logger().warning("Fair price unavailable — waiting for market data.")
            return

        self._last_fair_display = fair_usdt

        if not self._should_refresh(fair_usdt):
            return
        ...
        proposal = self._create_proposal(fair_usdt)
        if not proposal:
            return
        ...
        self._last_fair_usdt = fair_usdt
        self._last_refresh_ts = self.current_timestamp

        bid_usdt = next((o.price for o in proposal if o.order_side == TradeType.BUY), None)
        ask_usdt = next((o.price for o in proposal if o.order_side == TradeType.SELL), None)

        self.logger().info(
            f"Refreshed quotes | fair_usdt={fair_usdt:.2f} USDT | "
            f"bid_usdt={self._fmt(bid_usdt)} ask_usdt={self._fmt(ask_usdt)}"
        )

    def did_fill_order(self, event: OrderFilledEvent):
        # Hedge first: maker/hedge can share a pair (e.g. BTC-USDT on both venues).
        if event.order_id in self._hedge_order_ids:
            msg = (
                f"Hedge fill: {event.trade_type.name} {event.amount} {event.trading_pair} "
                f"@ {event.price:.4f} USDT"
            )
    ...
    def _compute_fair(self) -> Optional[Decimal]:
        hedge = self.connectors[self.config.hedge_connector]
        return self._safe_mid_price(hedge, self.config.hedge_trading_pair)

    def _should_refresh(self, fair_usdt: Decimal) -> bool:
        if self._last_fair_usdt is None:
            return True
        drift_bps = abs(fair_usdt - self._last_fair_usdt) / self._last_fair_usdt * Decimal("10000")
    ...
    def _create_proposal(self, fair_usdt: Decimal) -> List[OrderCandidate]:
        spread_factor = self.config.half_spread_bps / Decimal("10000")
        bid_price = fair_usdt * (Decimal("1") - spread_factor)
        ask_price = fair_usdt * (Decimal("1") + spread_factor)
    ...
    def format_status(self) -> str:
        ...
        if self._last_fair_display is not None:
            lines.append(f"\n  Fair BTC-USDT: {self._last_fair_display:.2f}")

        if self._last_fair_display is not None and self._last_fair_usdt is not None:
            drift = (
                abs(self._last_fair_display - self._last_fair_usdt)
                / self._last_fair_usdt
                * Decimal("10000")
            )
